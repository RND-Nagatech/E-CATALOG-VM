...
export const ProductGrid: React.FC<ProductGridProps> = ({
  ...
  // ✅ Samakan gridClass di semua kondisi
  const gridClass =
    "product-grid-container grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 custom1375:grid-cols-4 gap-4 sm:gap-6";
  ...
  {
    console.log("Rendering ProductGrid with products:", products.length);
  }
    ...
    <div className={gridClass}>
      {products.map((product) => (
        <ProductCard
          ...
          isInCart={cartItems?.some((p) => p.id === product.id) ?? false}
        />
      ))}
    </div>
  );
